...
export default function Home() {
    ...
    const startDrawing = (e: React.MouseEvent<HTMLCanvasElement>) => {
        const canvas = canvasRef.current;
        if (canvas) {
            saveToUndoStack(); // Save the current state to undo stack
            const ctx = canvas.getContext('2d');
            if (ctx) {
                ctx.beginPath();
    ...
    return (
        <>
            <div className='grid grid-cols-3 gap-2'>
                ...
                <Button onClick={undo} className='z-20 bg-black text-white' variant='default'>
                    Undo
                </Button>
                <Button onClick={redo} className='z-20 bg-black text-white' variant='default'>
                    Redo
                </Button>
                <Group my='sm' className='z-20'>
                    {SWATCHES.map((swatch) => (
                        <ColorSwatch key={swatch} color={swatch} onClick={() => setColor(swatch)} />
                    ))}
                </Group>
                <Button onClick={runRoute} className='z-20 bg-black text-white' variant='default'>
                    Run
                </Button>
            </div>
            <canvas
            ...
            ></canvas>
            {latexExpression.map((expr, index) => (
                <Draggable key={index} defaultPosition={latexPosition}>
                    <div className='latex absolute text-white text-2xl' dangerouslySetInnerHTML={{ __html: expr }} />
                </Draggable>
            ))}
        </>
    );
